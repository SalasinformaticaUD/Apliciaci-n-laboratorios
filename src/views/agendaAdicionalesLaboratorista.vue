      ...
      <v-sheet height="520" tile>
        ...
        >
          ...
          >
            ...
            <v-card-title>Información del laboratorio</v-card-title>    
            <v-card-text>
              <v-divider></v-divider>
      ...
      <v-dialog v-model="menuCreatedEvent" max-width="400px">
        ...
        >
          ...
          <v-card-text>
            ...
            <h4>3. Seleccione los bancos que desea habilitar</h4>
            <v-row no-gutters class="mx-2 mt-2">
              <v-col cols="12" sm="4" class="my-n3" v-for="(item,index) in disponibilidadBancos" :key="index">
                <v-checkbox
                  v-model="disponibilidadBancos[index]"
                  :label="`Banco ${index+1}`"
            ...
            <v-row no-gutters v-if="createdEvent.name">
              <v-col cols="12" sm="7" class="my-n3 mx-2">
                <v-checkbox
                  ...
                  :color="createdEvent.color"
                  @click="disponibilidadBancos.fill(selectAllBancos)"
                >
                </v-checkbox>
              </v-col>
            </v-row>
...
export default {
  ...
  watch:{
    disponibilidadBancos: function(){
      ...
      if(this.disponibilidadBancos !== null){
        let bancosDisponibles = this.disponibilidadBancos.filter(item => item===true).length;
        if(bancosDisponibles == this.disponibilidadBancos.length){
    ...
    stringNameLab: function(val){
      ...
      if (val!==""){
        ...
        if (this.flagEdicion === false){          
          this.disponibilidadBancos = new Array(filterInfoLab.bancos).fill(false);
        }
      }
    },
    labSelect: function(){
  ...
  methods:{
        ...
        if(event.bancos[i]!=="No Disponible"){
          ...
          if(event.bancos[i]=="Pendiente" || event.bancos[i]=="Reservado"){
            if(event.bancos[i]=="Pendiente"){
              ...
              this.auxEstadoBancos.Resrvados.push(i+1)
            }
    ...
    verificarBancosSeleccionados(){
      ...
      let numBancosDisponibles = this.disponibilidadBancos.filter(item => item==true).length;
      if(numBancosDisponibles > 0){
        console.log(this.disponibilidadBancos);
        if (this.flagEdicion === true){
          ...
          this.createdEvent.bancos = this.disponibilidadBancos.map(item => item===true?"Disponible":"No Disponible");
          this.verificarCruce();
        }
      }else{
    ...
    verificarCruce(){      
      // Si se esta agregando un nuevo laboratorio se verifica si coincide con alguno de los eventos para no repetir laboratorio en la misma franja horaria
